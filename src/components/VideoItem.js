...
export default function VideoItem({ data, handleEdit }) {
//  const { deleteItem, markAsDone, markActive } = useContext(ItemsContext);

  data.preview = data.preview || "http://192.168.2.83:8888/temp/3577.jpg";

  return (
    <div className="VideoItem">
      <div className="Preview">
        <div className="PreviewImage"><img src={ data?.preview || "NoImage.png" } /></div>
        <div className="VideoDuration">{ data?.duration || "0:00" }</div>
      </div>
      <div className="Title">{ data?.name || "Noname" }</div>
    </div>
  );
//  return (
